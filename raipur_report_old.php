...
// $sql = "SELECT * FROM tbl_document_master where railwayset=1 ORDER BY doc_id ASC";
...
// Base SQL query
$sql = "SELECT * FROM tbl_document_master WHERE railwayset=1";

// Apply search filter
if (!empty($search)) {
    $sql .= " AND (letter_no LIKE '%" . mysqli_real_escape_string($db_con, $search) . "%' 
                OR description LIKE '%" . mysqli_real_escape_string($db_con, $search) . "%'
                OR doc_path LIKE '%" . mysqli_real_escape_string($db_con, $search) . "%')";
}


// Apply date range filter
if (!empty($date1) && !empty($date2)) {
    $sql .= " AND DATE(dateposted) BETWEEN '" . mysqli_real_escape_string($db_con, $date1) . "' 
              AND '" . mysqli_real_escape_string($db_con, $date2) . "'";
}
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                if (!empty($row['doc_path'])) {
                                                    ...
                                                    echo '<a href="storageFiles?id=' . urlencode(base64_encode($data[sl_id])) . '" target="_blank">' . htmlspecialchars($fileName) . '</a>';
                                                } else {
                                                    echo 'No File';
                                                }
                                                echo '</td>';
