...
$conn = @mysqli_connect($dbHost, $dbUser, $dbPwd, $dbName) OR die('could not connect:' . mysqli_connect_error());
$clientIds = '241';
$dbresult = mysqli_query($conn, "select db_name from tbl_client_master where client_id in($clientIds)") or die('Error:' . mysqli_error($conn));

while($rowd =  mysqli_fetch_assoc($dbresult)){
	
	$SAAS_DIR = 'ezeefile_saas_client' . DIRECTORY_SEPARATOR . $rowd['db_name'];
	
	$DIR  = str_replace("eprocess", $SAAS_DIR, __DIR__);

	$db_con = @mysqli_connect($dbHost, $dbUser, $dbPwd, $rowd['db_name']) OR die('could not connect:' . mysqli_connect_error());
	...
	while($pending_doc = mysqli_fetch_assoc($fetchDoc)){
		...
		$filePath = $DIR . DIRECTORY_SEPARATOR . "extract-here" . DIRECTORY_SEPARATOR . $pending_doc['doc_path'];
