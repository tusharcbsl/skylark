...
//    }
    ...
    public function generatePathImagemagick($inputFile, $output, $outDIr, $docId) {
        ...
        for ($j = 0; $j < $pdfPage; $j++) {
            ...
            if ($im->writeImage($outputDir . 'ocr-' . $j . '.png')) {
//                if ($pdfPage > 1) {
                $strOutss = $this->outputDir . "/" . $docId . "/" . $j . ".txt";
                $strIn = $this->outputDir . "/" . $docId . "/ocr-" . $j . ".png";
                $textImage = (new TesseractOCR($strIn))
                        ->lang('eng', 'hin')
                        ->run();
                if (!empty($textImage)) {
...
//                }
    ...
    public function getImageOcr($imagePath) {
        $pdfPage = $this->count_pages($inputDir); //convert -density 250 -units PixelsPerInch
//              if ($pdfPage > 1) {
                $strIn = $imagePath;
                $strOutss='extract-here/test/';
                echo $strIn.'<br>';
                $textImage = (new TesseractOCR($strIn))
                        ->lang('eng', 'hin')
                        ->run();
...
//
$ocr = new OCR("extract-here/test");
//$ocr->ocrDone("extract-here/testthree/dummy.pdf", "", 14);
//$ocr->ocrDone("extract-here/test/onlineocr.png", "", 14);
$ocr->getImageOcr('extract-here/test/onlineocr.png');
