...
//                $coustomer_id = $response['customerid'];
        ...
        if ($create_client) {
            ...
            if ($client_status['status']) {
				...
				$qry_update_crm = mysqli_query($db_con, "select db_name from tbl_client_master where client_id='$lastinsertid'") or die("ERROR CLIENT DB FETCH" . mysqli_error($db_con));
                    $row = mysqli_fetch_assoc($qry_update_crm);
                    $dbname = $row['db_name'];
                    //mysqli_autocommit($db_con, TRUE);
                    echo '<script> taskSuccess("procedure?db=' . $dbname . '","Company created successfully"); </script>';
