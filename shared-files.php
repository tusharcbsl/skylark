    ...
    <body class="fixed-left">
        ...
        <div id="wrapper">
            ...
            <div class="content-page">
                    ...
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <ol class="breadcrumb">
                                    <li>
                                        <a href="shared-files"><?php echo $lang['YOUR_SHARED_DOCUMENTS_LIST']; ?></a>
                                    </li>
                        ...
                        <div class="panel">
                            <div class="box box-primary">
                                <div class="box-header with-border">
                                    <div class="col-sm-10">
                                        <div class="col-sm-4">
                                            <input type="text" class="form-control translatetext" name="shared_doc" id="shared_doc" value="<?php echo xss_clean($_GET['shared_doc']); ?>" parsley-trigger="change" placeholder="<?php echo $lang['search_shared_doc']; ?>" required />
                                        </div>
                                        <div class="col-sm-3">
                                            <button type="submit" class="btn btn-primary" id="shareddoc"><i class="fa fa-search"></i>  <?php echo $lang['Search']; ?></button>
                                            <a href="shared-files" class="btn btn-warning"><i class="fa fa-refresh"></i>  <?php echo $lang['Reset']; ?></a>
                                        </div>
                                ...
                                <div class="panel-body">
                                    ...
                                    if (isset($_GET['shared_doc']) && !empty($_GET['shared_doc'])) {
                                        ...
                                        $searchTxt = xss_clean($searchTxt);
                                        $searchTxt = mysqli_real_escape_string($db_con, $searchTxt);
                                        $where .= " and tdm.old_doc_name like '%$searchTxt%' and tds.from_id = '$loginUser' and flag_multidelete='1'";
                                    }
                                    $sql = "SELECT tdm.doc_id,tdm.doc_name,tdm.doc_extn,tdm.noofpages,tdm.old_doc_name,tds.id,tds.from_id,tds.to_ids,tds.doc_ids,tds.share_date  FROM `tbl_document_share` tds INNER JOIN `tbl_document_master` tdm ON tds.doc_ids=tdm.doc_id $where";
                                        ...
                                        <div class="container">
                                            <div class="box-body limit">
                                                ...
                                                <table class="table table-striped table-bordered js-sort-table">
                                                    <?php
                                                    mysqli_set_charset($db_con, "utf8");
                                                    $ShDocId = mysqli_query($db_con, "SELECT tdm.doc_id,tdm.doc_name,tdm.doc_extn,tdm.noofpages,tdm.old_doc_name,tds.id,tds.from_id,tds.to_ids,tds.doc_ids,tds.share_date FROM `tbl_document_share` tds INNER JOIN `tbl_document_master` tdm ON tds.doc_ids=tdm.doc_id $where order by tdm.old_doc_name asc LIMIT $start, $per_page") or die('Error in share id fetch' . mysqli_error($db_con));
                                                    if (mysqli_num_rows($ShDocId) > 0) {
                                                        ...
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th class="sort-js-date" ><?php echo $lang['Share_Date']; ?></th>
                                                            ...
                                                            while ($rwShDocId = mysqli_fetch_assoc($ShDocId)) {
                                                                ...
                                                                <tr class="gradeX">
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        if(file_exists('thumbnail/'.base64_encode($rwShDocId['doc_id']).'.jpg')){ ?><div> <img class="thumb-image" src="thumbnail/<?=base64_encode($rwShDocId['doc_id'])?>.jpg"> </div>
                                                                        <?php } 
                                                                    ...
                                                                    <td><?php echo $rwShDocId['share_date']; ?></td>
                                                                    <td class="actions">
                ...
                <?php
                if (isset($_POST['undoshare']) && intval($_POST['undo']) && intval($_POST['did'])) {
                    $id = intval($_POST['undo']); //user_id
                    $delDoc = intval($_POST['did']);
                    ...
                    $undoShare = mysqli_query($db_con, "delete from `tbl_document_share` where to_ids='$id' and doc_ids='$delDoc'"); // or die('Error:' . mysqli_error($db_con));
                    if ($undoShare) {
                        $log = mysqli_query($db_con, "insert into tbl_ezeefile_logs(`id`, `user_id`, `user_name`, `group_id`, `sl_id`, `action_name`, `start_date`, `end_date`, `system_ip`, `remarks`) values (null, '$_SESSION[cdes_user_id]', '$_SESSION[admin_user_name] $_SESSION[admin_user_last]',null,null,'You undo share ($rwundoDocNm[old_doc_name]) Document from $rwUserName[first_name] $rwUserName[last_name]'  ,'$date',null,'$host',null)"); // or die('error : ' . mysqli_error($db_con));
