...
class OfficeConverter {
     ...
     * @param string $bin
     */
    // $bin="libreoffice";//linux path change when use in linux
    
    public function __construct($filename, $tempPath = null, $bin = 'C:\LibreOffice\program\soffice') {
    ...
    public function convertTo($filename) {
        ...
        $this->makeCommand($outdir, $outputExtension);
		
		/* echo $this->makeCommand($outdir, $outputExtension) . " 2>&1";
		die();  */ 
        $shell = $this->exec($this->makeCommand($outdir, $outputExtension) . " 2>&1");
    ...
    protected function makeCommand($outputDirectory, $outputExtension) {
       ...
       // return "{$this->bin} --headless --convert-to {$outputExtension} {$oriFile} --outdir {$outputDirectory}";
        return "{$this->bin} --headless --norestore --writer --convert-to {$outputExtension} {$oriFile} --outdir {$outputDirectory}";
    }
