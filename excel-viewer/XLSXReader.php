	...
	public function getSheet($sheet) {
		...
		if(!array_key_exists($sheet, $this->sheets)) {
			$this->sheets[$sheet] = new XLSXWorksheet($this->getSheetXML($sheet), $sheet, $this);
                        $ob_str= serialize($this->sheets[$sheet]); 
                        if(strpos($ob_str, 'no data found sk@130219')){
                            return FALSE;
                        }
...
class XLSXWorksheet {
	...
	protected function parse($xml) {
            //var_dump($xml); die('jkkjj');
		$st=$this->parseDimensions($xml->dimension);
                //var_dump($st);die;
                if(!$st){
                    $this->parseData($xml->sheetData);
                }else{
                    $this->is_empty ='no data found sk@130219';
                    return;
                }
	...
	protected function getColumnIndex($cell = 'A1') {
		...
		throw new Exception("Invalid cell index".$cell);
	}
