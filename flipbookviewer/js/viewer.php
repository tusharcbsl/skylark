...
var PDFPageView = (function PDFPageViewClosure() {
 ...
 var wmCanvas=document.createElement("canvas");
            ...
            wmContext.globalAlpha=0.6;
          // setup text for filling
            wmContext.font = "20px sans-serif" ;
            wmContext.fillStyle = "rgba(66, 66, 66, 0.5)";
           // get the metrics with font settings
            var metrics = wmContext.measureText("Web page, should not be printed.");
            var width = metrics.width;
           // height is font size
            var height = 24;
           // change the origin coordinate to the middle of the context
              wmContext.translate(viewport.width/2, 300);
           ...
           // // as the origin is now at the center, just need to center the text
            wmContext.fillText("Web copy, not to be printed.",-width/2,height/2);
            if(div.firstChild)
    ...
    beforePrint: function PDFPageView_beforePrint(printContainer) {
     ...
     var cwi=Math.floor(viewport.width) * PRINT_OUTPUT_SCALE;
     //cwi=cwi.replace('px','');
     var cwh=Math.floor(viewport.height) * PRINT_OUTPUT_SCALE;
     //cwh=cwh.replace('px','');
     waterMark.width = cwi;
      ...
      waterMark.setAttribute("style","position:absolute;border:0px solid;z-index:10000;");
        var wmContextw=waterMark.getContext('2d');
          ...
          <?php switch($wpos){
            ...
            ?>
                //rotate the context (so it's rotated around its center)
                wmContextw.rotate(-Math.atan(wmCanvas.height/wmCanvas.width));
            <?php } ?>
           // change the origin coordinate to the middle of the context
              /*wmContextw.translate(cwi/2, cwi/2);
            //rotate the context (so it's rotated around its center)
...
function configure(PDFJS) {
  PDFJS.imageResourcesPath = './images/';
  PDFJS.workerSrc = 'flipbookviewer/js/pdf.worker.js';
  PDFJS.cMapUrl = '/flipbookviewer/cmaps/';
  PDFJS.cMapPacked = true;
}
