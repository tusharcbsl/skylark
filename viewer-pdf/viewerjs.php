...
//ab@210621 dynamic watermark
$wm_setting = mysqli_query($db_con, "CALL select_data('tbl_watermark_setting','*','','')");
$db_con->next_result();
$fetch_wmsetting = mysqli_fetch_assoc($wm_setting);
if($fetch_wmsetting['is_enable']=='1'){
    if($fetch_wmsetting['is_customized']=='1'){
        $wm_master = mysqli_query($db_con, "CALL select_data('tbl_watermark_master','*','','')");
        $db_con->next_result();
        $fetch_wm_master = mysqli_fetch_assoc($wm_master);

        $watermark_content = $fetch_wm_master['watermark'];
        $size = $fetch_wm_master['size'];
        $fcolor = hexToRgb($fetch_wm_master['fcolor']);
        $wpos = $fetch_wm_master['w_position'];
        $wrot = $fetch_wm_master['w_rotation'];


    }elseif($fetch_wmsetting['is_customized']=='0'){
        ...
        if(!empty($_SESSION['custom_ip'])){
           $clientip=$host.'/'.$_SESSION[custom_ip];
        }
        $username=$_SESSION['adminMail'];
        $username=substr($username,0,strrpos($username,'@'));
        $dcid = $_GET['i'];
        $docName=$rwFile['old_doc_name'];
        $slid=$rwFile['doc_name'];
        $docExtn=$rwFile['doc_extn'];

        $watermark_content = $username.'@'.$clientip;
        $size = '50';
        $fcolor['r'] = '66';
        $fcolor['g'] = '66';
        $fcolor['b'] = '66';
...
}else if($fetch_wmsetting['is_enable']=='0'){
    $watermark_content = '';
    $size = '0';
    $fcolor['r'] = '66';
    $fcolor['g'] = '66';
 ...
 */
'use strict';
var DEFAULT_URL = '<?php echo $localPath; ?>';
;
 ...
 (function () {
     ...
     draw: function PDFPageView_draw() {
      ...
      this.textLayer = textLayer;
       ...
       var wmCanvas=document.createElement("canvas");
            ...
            var cwh=wmCanvas.height;
            var cwi=wmCanvas.width;

            wmCanvas.setAttribute("style","position:absolute;border:0px solid")
            var wmContext=wmCanvas.getContext('2d');
            wmContext.globalAlpha=0.4;
            // setup text for filling
            wmContext.font = "<?=$size?>px sans-serif" ;
            // console.log(cwh*4/100);
            wmContext.fillStyle = "rgba(<?=$fcolor['r']?>, <?=$fcolor['g']?>, <?=$fcolor['b']?>, 0.5)";
            // get the metrics with font settings
            var metrics = wmContext.measureText("<?php echo $watermark_content;?>");
            ...
            <?php switch($wpos){
                case 0: ?>
                    //top left
                    wmContext.translate(cwi/4, cwh/8);
                    <?php break;
                case 1: ?>
                    //top center
                    wmContext.translate(cwi/2, cwh/8);
                    <?php break;
                case 2: ?>
                    //top right
                    wmContext.translate(cwi/1.2, cwh/8);
                    <?php break;
                case 3: ?>
                    //middle left
                    wmContext.translate(cwi/4, cwh/2);
                    <?php break;
                case 4: ?>
                    //middle center
                    wmContext.translate(cwi/2, cwh/2);
                    <?php break;
                case 5: ?>
                    //middle right
                    wmContext.translate(cwi/1.2, cwh/2);
                    <?php break;
                case 6: ?>
                    //bottom left
                    wmContext.translate(cwi/4, cwh/1.2);
                    <?php break;
                case 7: ?>
                    //bottom center
                    wmContext.translate(cwi/2, cwh/1.2);
                    <?php break;
                case 8: ?>
                    //bottom right
                    wmContext.translate(cwi/1.2, cwh/1.2);
                    <?php break;    
                default: ?>
                    //middle center
                    wmContext.translate(cwi/2, cwh/2);
            ...
            wmContext.fillText("<?php echo $watermark_content;?> ",-width/2,height/2);
            if(div.firstChild)
  ...
  (function (root, factory) {
   ...
   function configure(PDFJS) {
    ...
    PDFJS.workerSrc = 'viewer-pdf/build/pdf.worker.js';
    PDFJS.cMapUrl = './cmaps/';
   ...
   var PDFViewerApplication = {
    ...
    setTitle: function pdfViewSetTitle(title) {
     ...
     document.title = '<?php echo $docName;?>';
    },
  ...
  (function (root, factory) {
   ...
   PDFPrintService.prototype = {
    ...
    useRenderedPage: function (printItem) {
     ...
     //watermark on print
     /*var imgWater = document.createElement('img');
     imgWater.style.width = printItem.width;
     imgWater.style.height = printItem.height;
     imgWater.src="logo.png";*/
     //var imgWater=document.getElementById("waterImage");
     var waterMark = document.createElement('canvas');
     var cwi=printItem.width;
     cwi=cwi.replace('px','');
     var cwh=img.style.height;
     cwh=cwh.replace('px','');
     waterMark.width = cwi;
     waterMark.height = cwh;
     //alert(cwi+'/'+cwh);
      waterMark.setAttribute("style","position:absolute;border:0px solid;z-index:10000;");
        var wmContextw=waterMark.getContext('2d');
            wmContextw.globalAlpha=0.4;
             wmContextw.font = "<?=$size?>px Arial" ;
           wmContextw.fillStyle = "rgba(<?=$fcolor['r']?>, <?=$fcolor['g']?>, <?=$fcolor['b']?>, 1)";
           
           // get the metrics with font settings
           var metrics = wmContextw.measureText("<?php echo $watermark_content;?> ");
           var width = metrics.width;
           // height is font size
            var height = 200;
            ...
            <?php switch($wpos){
                case 0: ?>
                    //top left
                    wmContextw.translate(cwi/4, cwh/8);
                    <?php break;
                case 1: ?>
                    //top center
                    wmContextw.translate(cwi/2, cwh/8);
                    <?php break;
                case 2: ?>
                    //top right
                    wmContextw.translate(cwi/1.2, cwh/8);
                    <?php break;
                case 3: ?>
                    //middle left
                    wmContextw.translate(cwi/4, cwh/2);
                    <?php break;
                case 4: ?>
                    //middle center
                    wmContextw.translate(cwi/2, cwh/2);
                    <?php break;
                case 5: ?>
                    //middle right
                    wmContextw.translate(cwi/1.2, cwh/2);
                    <?php break;
                case 6: ?>
                    //bottom left
                    wmContextw.translate(cwi/4, cwh/1.2);
                    <?php break;
                case 7: ?>
                    //bottom center
                    wmContextw.translate(cwi/2, cwh/1.2);
                    <?php break;
                case 8: ?>
                    //bottom right
                    wmContextw.translate(cwi/1.2, cwh/1.2);
                    <?php break;    
                default: ?>
                    //middle center
                    wmContextw.translate(cwi/2, cwh/2);
            ...
            ?>
                //rotate the context (so it's rotated around its center)
                wmContextw.rotate(-Math.atan(waterMark.height/waterMark.width));
            <?php } ?>
           ...
           /*   wmContextw.translate(cwi/2, cwh/2);
            //rotate the context (so it's rotated around its center)
             wmContextw.rotate(-Math.atan(waterMark.height/waterMark.width));*/
           // // as the origin is now at the center, just need to center the text
            wmContextw.fillText("<?php echo $watermark_content;?> ",-width/2,height/2);
            //show image in watermark
            //wmContextw.drawImage(imgWater,-width/2,height/2);
     //img1.src="logo.png";
     console.log(waterMark);
      this.printContainer.appendChild(waterMark);
      // end watermark print
...
function getViewerConfiguration() {
 return {
  ...
  documentProperties: {
   ...
   closeButton: document.getElementById('documentProperti\n\esClose'),
   fields: {
...
function downloadWithAnnotation(){
    
    
   $.post("viewer-pdf/downloadpdfwithannot.php", {docid:"<?php echo $dcid; ?>",docname:"<?php echo $docName; ?>",slid:"<?php echo $slid;?>",docPath:"<?php echo $localPath; ?>",docExtn:"<?php echo $docExtn;?>"}, function (result, status) {
        if (status == 'success') {
            // debugger;
           //alert(result);
            // $("#child1").html(result);
            //alert(result);
            var fileURL='viewer-pdf/'+result;
            $('a[href$="'+fileURL+'"]').attr('download', '').attr('target', '_blank'); 
           window.open(fileURL, '_blank')
            //download_file("viewer-pdf","viewer-pdf/"+result);
            }
           });   
}
