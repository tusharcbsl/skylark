                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    <div class="container">
                                        <div class="row">
                                            <div class="col-sm-12">
                                                <?php if ($rwgetRole['add_group'] == '1') { ?>
                                                    <div class="col-sm-3">
                                                        <a href="javascript:void(0)" class="btn btn-primary waves-effect waves-light" data-toggle="modal" data-target="#group-add"><?php echo $lang['Add_Nw_Grp']; ?> <i class="fa fa-plus"></i></a>
                                                    </div>
                                                <?php } ?>

                                                <form method="get">
                                                    <div class="col-sm-4">
                                                        <input type="text" class="form-control" name="grpname" value="<?php echo xss_clean($_GET['grpname']); ?>" parsley-trigger="change"  data-parsley-required-message="Enter Group Name for Search"placeholder="<?php echo $lang['Search_Group']; ?>"  />
                                                    </div>
                                                    <div class="col-sm-3">
                                                        <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i> <?php echo $lang['Search']; ?></button>
                                                        <a href="groupList" class="btn btn-warning"><i class="fa fa-refresh"></i> <?php echo $lang['Reset']; ?></a>
                                                    </div>
                                        ...
                                        <div class="row">
                                            <?php
                                            $where = "where group_id in($sameGroupIDs)";
                                            if (isset($_GET['grpname']) && !empty($_GET['grpname'])) {
                                                //$groupName = preg_replace("/[^A-Za-z0-9 ]/", "", $_GET['grpname']);
                                                $groupName  = xss_clean(trim($_GET['grpname']));
                                                $groupName = preg_replace('/[^\w$\x{0080}-\x{FFFF}@#$&!%()_ <>]+/u', "", $groupName);
                                                $where .= "and group_name LIKE '%$groupName%'";
                                            }
                                            mysqli_set_charset($db_con, "utf8");
                                            $sql = "SELECT * FROM  tbl_group_master $where";
                                            ...
                                            if ($foundnum > 0) {
                                                ...
                                                <div class="box-body">
                                                    ...
                                                    <?php
                                                    mysqli_set_charset($db_con, "utf8");
                                                    $users = mysqli_query($db_con, "select * from tbl_group_master $where order by group_name asc LIMIT $start, $per_page") or die('Error:' . mysqli_error($db_con));
//                                             
                                                        ...
                                                        <ul class='pagination strgePage'>
                                                            ...
                                                            if ($max_pages < 7 + ($adjacents * 2)) {   //not enough pages to bother breaking it up
                                                                ...
                                                                for ($counter = 1; $counter <= $max_pages; $counter++) {
                                                                    if ($i == $start) {
                                                                        echo "<li class='active'><a href='?start=$i&limit=$per_page&grpname=" . $groupName . "''><b>$counter</b></a> </li>";
                                                                    } else {
                                                                        echo "<li><a href='?start=$i&limit=$per_page&grpname=" . $groupName . "''>$counter</a></li> ";
                                                                    }
                                                                    $i = $i + $per_page;
                                                                }
                                        ...
                                        <div class="modal-body" >
                                            ...
                                            <div class="row">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="groupName" required class="form-control" id="groupName1" placeholder="<?php echo $lang['group_Name']; ?>">
                                                </div>
                                            </div>
                                            <div class="row"> 

                                                <div class="form-group">
                                                    <label><?php echo $lang['Add_Users_in_Group']; ?></label>
                                                    <select class="select2 select2-multiple" multiple data-placeholder="<?php echo $lang['Slt_Usrs']; ?>" name="userid[]">
                                                        <?php
                                                        $user = mysqli_query($db_con, "select user_id,first_name,last_name from tbl_user_master where user_id!='1' and user_id!='$_SESSION[cdes_user_id]' order by first_name,last_name asc");
                ...
                <script type="text/javascript">
                                        $(document).ready(function () {
                                            $('form').parsley();
                                            $('#groupName1').on("cut copy paste", function (e) {
                                                e.preventDefault();
                                            });
                                            addTranslationClass();
                                        });
                                        ...
                                        $(function () {
                                            ...
                                            $('input#groupName1').keyup(function ()
                                            {
                                        ...
                                        //groupname
                                        $("input#groupName1").keypress(function (e) {
                ...
                <script>
                    ...
                    $("a#editRow").click(function () {
                        ...
                        values = $row.find('td:nth-child(2)').map(function () {
                            ...
                            $.post("application/ajax/updateGroup.php", {ID: $id, token:token}, function (result, status) {
                                if (status == 'success') {
                ...
                <?php
                if (isset($_POST['addGroup'],$_POST['token'])) {
                    $userid = $_POST['userid'];
                    $userid[] = '1';
                    ...
                    $groupName = xss_clean($_POST['groupName']);
                    $groupName = preg_replace('/[^\w$\x{0080}-\x{FFFF}@#$&!%()_ <>]+/u', "", $groupName);
                    ...
                    if (mysqli_num_rows($check) <= 0) {
                        ...
                        echo'<script>taskFailed("' . basename($_SERVER['REQUEST_URI']) . '","' . $lang['Group_Already_Exist'] . '!");</script>';
                    }
                    mysqli_close($db_con);
                }

                if (isset($_POST['editGroup'],$_POST['token'])) {
                   ...
                   // $groupName = preg_replace("/[^a-zA-Z0-9 ]/", "", $groupName); //filter groupname
                    ...
                    if (mysqli_num_rows($fetchValidate) > 0) {
                        echo'<script>taskFailed("' . basename($_SERVER['REQUEST_URI'])  . '","Group Already Exist !");</script>';
                    } else {
                        mysqli_set_charset($db_con, "utf8");
                        ...
                        if ($edit) {
                            ...
                            echo'<script>taskSuccess("' . basename($_SERVER['REQUEST_URI'])  . '","' . $lang['Group_Updated_Successfully'] . ' !");</script>';
                        }
                ...
                if (isset($_POST['delete'],$_POST['token'])) {
                    ...
                    if ($del) {
                        ...
                        echo'<script>taskSuccess("' . basename($_SERVER['REQUEST_URI'])  . '","' . $lang['Group_Deleted_Successfully'] . ' !");</script>';
                    }
                    mysqli_close($db_con);
                }
                ?>
                                ...
                                <tr class="gradeX">
                                    ...
                                    <td class="actions">
                                        <?php if ($rwgetRole['modify_group'] == '1') { ?>
                                            ...
                                            if ($rwUser['group_name'] == 'Administrator' && $_SESSION['cdes_user_id'] != '1') {
                                                echo 'display:none';
                                            }
