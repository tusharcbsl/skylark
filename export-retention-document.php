...
if (isset($_POST['recyclefile']) && !empty($_POST['recyclefile'])) {

    $deleteFile = trim($_POST['recyclefile']);
...
if (isset($_POST['exportUser'], $_POST['token'])) {
    ...
    $exportData = mysqli_query($db_con, "SELECT old_doc_name, retention_period, doc_extn, doc_name, doc_size FROM `tbl_document_master` $where order by old_doc_name") or die('Error:' . mysqli_error($db_con));

    if ($selectFormat == "xlsx") {

        $header1 = 'Sr. No., File name, Retention Time, File Type, Name of Storage, File Size';
        $i=1;
