...
//check email notification time before
    ...
    foreach ($expirytime as $docexptime) {
        ...
        if (strtotime($notifydays_ago) < strtotime($docexptime) && strtotime($docexptime) > strtotime($today)) {
            ...
            $mailbodyhtml = expiryDocumentlistView($db_con, $notifydays_before, $date);
            if ($mailbodyhtml) {
            ...
            $Expdocmailbody = expiryDocumentlist($db_con, $date);
            if ($Expdocmailbody) {
                $expflag = 1;
    ...
    if ($expflag == 1) {
        ...
        $schedularlog = mysqli_query($db_con, "select old_doc_name,doc_name,doc_id from tbl_document_master  WHERE doc_expiry_period <='$date' and flag_multidelete='1'");
...
function expiryDocumentlistView($db_con, $notifydays_before, $date) {
	
    $getdocument = mysqli_query($db_con, "SELECT * FROM `tbl_document_master` WHERE doc_expiry_period <='$notifydays_before' AND doc_expiry_period > '$date' AND doc_expiry_period IS NOT NULL and doc_expiry_period!='' and flag_multidelete='1'");
...
function expiryDocumentlist($db_con, $date) {

    $getdocument = mysqli_query($db_con, "SELECT * FROM `tbl_document_master` WHERE doc_expiry_period <='$date' AND doc_expiry_period IS NOT NULL and doc_expiry_period!='' and flag_multidelete='1'");
    if (mysqli_num_rows($getdocument) > 0) {
        $html = '<table border="1" cellpacing="2" cellpadding="8" style="border-collapse : collapse;">';
        $html .= '<tr>';
        $html .= '<th>SNo.</th>';
