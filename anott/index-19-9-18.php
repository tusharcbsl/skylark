...
<html>
    ...
    <body>
        ...
        <div class="toolbar">
           <div class="row">
            <div class="col-sm-6">
              <button data-toggle="modal" data-target="#con-close-modal-insert" class="btn btn-primary btn-sm add m-l-5" id="insert_file" style="margin-top:4px;" title="Add New Page"><i class="fa fa-plus"></i>  Add
                </button>
           <?php if($rwgetRole['delete_page']=='1'){?>
            <div class="pull-left">
                <form method="post">
                    <button type="button" data-target="#con-close-modal-del" title="Delete This Page from PDF File." data-toggle="modal" name="del-btn"  value="Delete"  class="btn btn-danger btn-sm del m-l-5" id="del-btn" style="margin-top:4px;" ><i class="fa fa-trash-o"></i> Delete
                    </button>
                </form>
            </div>
          <?php } ?>
          ...
          <?php if($rwgetRole['wf_log']=='1' || $rwgetRole['review_log']=='1'){?>  
            <div class="panel-body">
                <ul class="nav nav-pills">
                    ...
                    <li class="active tbs"><a href="#navpills-1" data-toggle="tab" aria-expanded="true">Workflow Log</a></li>
                    <?php }?>
                    <?php if($rwgetRole['review_log']=='1'){ ?>
                    <li class="<?=($rwgetRole['wf_log']!='1' ? 'active':'')?> tbs"><a href="#navpills-2" data-toggle="tab" aria-expanded="false">Review Log</a></li>
                     <?php }?>
                </ul>
              ...
              <div id="navpills-1" class="tab-pane active">
                ...
                echo '<center>No workflow log found</center><hr style="color:#000;">';
              }
              ?>   
             </div>
            <?php }?>
            <?php if($rwgetRole['review_log']=='1'){?>
                <div id="navpills-2" class="tab-pane <?=($rwgetRole['wf_log']!='1' ? 'active' : '')?>">
                 <?php
                 $rlog_sql="select rl.*,u.first_name,u.last_name from tbl_reviews_log rl left join tbl_user_master u on rl.user_id=u.user_id where 1=1 ";
                  if($_SESSION[cdes_user_id]!=1){
                  ...
                  if(mysqli_num_rows($rlog_query)>0){
                   ...
                   echo '<span>' . $rlog_res['action_name'] . '<br>';
                   echo '<strong> Action By :</strong> ' . $rlog_res['first_name'] .' '.$rlog_res[last_name].'<br>';
                   echo '<strong> Action Time :</strong>' . date('d M Y H:i:s',strtotime($rlog_res['start_date'])) . '</span>';
                    }
                }
                else{
