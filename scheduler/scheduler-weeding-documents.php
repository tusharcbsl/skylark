...
//print_r($retentionirytime); die;
    ...
    foreach ($retentionirytime as $docretentiontime) {
        ...
        if (strtotime($notifydays_ago) < strtotime($docretentiontime) && strtotime($docretentiontime) > strtotime($today)) {
            ...
            $mailbodyhtml = RetentionDocumentlistView($db_con, $notifydays_before);
            if ($mailbodyhtml) {
                $retentiondocflag = 2;
            }else{
            ...
            $retentiondocmailbody = RetentionDocumentlist($db_con);
...
function RetentionDocumentlistView($db_con, $notifydays_before) {
    $getdocument = mysqli_query($db_con, "SELECT * FROM `tbl_document_master` WHERE retention_period <='$notifydays_before' AND retention_period > NOW() AND retention_period IS NOT NULL and retention_period!='' and (flag_multidelete='1' or flag_multidelete='2')");
...
function RetentionDocumentlist($db_con) {
    $getdocument = mysqli_query($db_con, "SELECT * FROM `tbl_document_master` WHERE retention_period <=NOW() AND retention_period IS NOT NULL and retention_period!='' and (flag_multidelete='1' or flag_multidelete='2')");
    if (mysqli_num_rows($getdocument) > 0) {
        $html = '<table border="1" cellpacing="2" cellpadding="8" style="border-collapse : collapse;">';
        $html .= '<tr>';
        $html .= '<th>SNo.</th>';
...
function deleteDocumentRetentionPeriodOver($db_con, $date, $host, $fileserver, $port, $ftpUser, $ftpPwd) {
	echo "ddd";
    $retentiondoc = mysqli_query($db_con, "SELECT * FROM `tbl_document_master` where retention_period <= NOW() AND retention_period IS NOT NULL AND retention_period!='' AND (flag_multidelete='1' or flag_multidelete='2')");
    while ($rwretentiondoc = mysqli_fetch_assoc($retentiondoc)) {
        ...
        if (FTP_ENABLED) {
            ...
            //echo ROOT_FTP_FOLDER . '/' . $filePath; die;
