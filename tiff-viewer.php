...
$status = 0;
$checkfileLockqry = mysqli_query($db_con, "SELECT * FROM `tbl_locked_file_master` WHERE doc_id='$docId' and is_active='1'");
...
// assign doc id
   ...
   $fileManager = new fileManager();
	...
	//for tiff viewer @dv 21-02-2019
    $tiffOrginalPath = $localPath;
    $systemPath = __DIR__ . '/' . $localPath;
    $pdfPath = explode(".", $systemPath);
    $tiffPath = explode(".", $localPath);
    $tiff = tiffTopdf($systemPath, $pdfPath[0] . ".pdf"); // or die(print_r(error_get_last()));
    if ($tiff) {
        //open pdf viewer   
        $localPath = $tiffPath[0] . ".pdf";
    } else {
        die("Unable to view this TIFF file");
    }
	
	
//log file view
    $log = mysqli_query($db_con, "insert into tbl_ezeefile_logs(`id`, `user_id`, `user_name`, `group_id`, `sl_id`, `action_name`, `start_date`, `end_date`, `system_ip`, `remarks`,`doc_id`) values (null, '$_SESSION[cdes_user_id]', '$_SESSION[admin_user_name] $_SESSION[admin_user_last]',null,'$slid','Document $fileName viewed by $_SESSION[adminMail]','$date',null,'$host','','$docId')"); // or die('Error : ' . mysqli_error($db_con));
    $did = urlencode(base64_encode($docId));
                        ...
                        <div id="toolbarContainer">
                            <div id="toolbarViewer">
                                <div id="toolbarViewerLeft">
                                    ...
                                    <!--                                <button type="button"  title="Next Page" id="comment_button" style="color: #ffffff; margin-top: 3px;">Comment</button>-->


                                </div>

                                <div id="toolbarViewerRight">
                                    <!--                                <button type="button"  title="Show log for this file." id="log" style="color: #ffffff; margin-top: 3px;">Log</button>-->
    ...
    // When the user clicks anywhere outside of the modal, close it
            ...
            window.onbeforeunload = function () {
                ...
                $.post("application/ajax/removeTempFiles.php", {filepath: "<?php echo '../' . $localPath; ?>", filepath2: "<?php echo '../' . $tiffOrginalPath; ?>"}, function (result) {
