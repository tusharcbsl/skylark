...
class csrfToken{
    public static function generate(){
       ...
       // setcookie("ezeetoken", $token, time()+3600);
...
function xss_clean($str, $is_image = FALSE)
{
        ...
        $entrydata = $security->xss_clean($str, $is_image);
        if(!is_array($entrydata)){
            $entrydata = trim(preg_replace('/ +/', ' ', urldecode(html_entity_decode(strip_tags($entrydata)))));
