...
if (isset($_POST['bulkUpload'], $_POST['token'])) {
            ...
            if ($okay) {
                ...
                if ($fileUpload) {
                    ...
                    if (mysqli_num_rows($check) > 0) {
                        fwrite($logs, '<p class="text-danger">' . $i . '. ' . $date . " : $fileNameExtn : already exist in $storageName storage.\n</p>");
                        $message[] = "$fileNameExtn already exist in $storageName storage";
                            ...
                            $fileins = mysqli_query($db_con, $query) or die('Error22' . mysqli_error($db_con));
                            $upload = 1;
	...
	if ($upload) {
		...
		foreach ($sourcePath as $key => $value) {
			// 256 encryption of file before transfer to file server
			encrypt_my_file($sourcePath[$key]);

			if ($fileManager->uploadFile($sourcePath[$key], $destinationPath[$key])) {
